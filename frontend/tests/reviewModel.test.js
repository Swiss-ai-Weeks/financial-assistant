...
import { createReview, updateReview, graphCounts, filterGraph, provenanceFor, requirementCoverage, summaryFor, sourceCategory, serializeReview, restoreReview } from '../src/reviewModel.js';
...
test('illustrative fixture has support, counter, assumptions, gaps, calculations and source lineage', () => {
  const demo = JSON.parse(readFileSync(new URL('../public/investigation_review_demo.json', import.meta.url)));
...
test('every saved graph keeps valid endpoints, unique IDs and can create a review', () => {
  for (const file of ['investigation_demo.json','investigation_review_demo.json','investigation_live_nvidia.json','investigation_gs_ual_2026-03-20.json','investigation_axp_bac_2026-02-27.json']) {
    const g = JSON.parse(readFileSync(new URL(`../public/${file}`, import.meta.url)));
