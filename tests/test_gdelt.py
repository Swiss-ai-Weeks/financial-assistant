...
def test_content_farms_already_in_an_archive_are_not_served(tmp_path):
    rows = [
        {"url": "https://cnbc.com/a", "title": "Real", "seendate": "20260901T120000Z", "domain": "cnbc.com"},
        {"url": "https://tickerreport.com/b", "title": "Templated", "seendate": "20260901T120000Z", "domain": "tickerreport.com"},
    ]

    (tmp_path / "BAC.jsonl").write_text("".join(json.dumps(r) + "\n" for r in rows))

    items = GdeltNewsSource(GdeltArchive(tmp_path)).fetch("BAC", "")
...
def test_download_is_sliced_resumable_and_deduplicated(tmp_path):
    gdelt = FakeGdelt()
    archive = GdeltArchive(tmp_path, gdelt.client())
    ...
    added = archive.download("bac", "Bank of America Corporation", **window)
    ...
    assert archive.download("BAC", "Bank of America Corporation", **window) == 0
    assert len(gdelt.requests) == 4

    # A wider window only requests the slices it lacks.
    window["end"] = datetime(2026, 3, 6, tzinfo=UTC)

    assert archive.download("BAC", "Bank of America Corporation", **window) == 1
...
def test_open_slice_is_downloaded_again_next_time(tmp_path):
    ...
    archive.download("BAC", "Bank of America Corporation", **window)
...
def test_source_serves_the_archive_without_touching_the_network(tmp_path):
    ...
    GdeltArchive(tmp_path, gdelt.client()).download(
        "BAC",
    ...
    def offline(url):
        raise AssertionError("the desk must never call GDELT")

    source = GdeltNewsSource(GdeltArchive(tmp_path, GdeltClient(http_get=offline)))
...
def test_archive_is_merged_into_the_wire_on_every_read(tmp_path):
    gdelt = FakeGdelt()
    archive = GdeltArchive(tmp_path / "gdelt", gdelt.client())

    news = NewsRepository(
        tmp_path / "cache",
        (GdeltNewsSource(archive),),
        cache_minutes=60,
    )
