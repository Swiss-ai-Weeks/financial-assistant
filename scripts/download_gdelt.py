"""
Download GDELT news into the local archive.
...
The desk never calls GDELT itself. It reads this archive, so
the download can take as long as GDELT's rate limit demands
(one request every five seconds) and can be interrupted and
resumed: finished slices are not requested again.
"""
...
def main() -> None:
    ...
    archive = get_gdelt_archive()

    print(f"GDELT {start:%Y-%m-%d} -> {end:%Y-%m-%d}, {len(tickers)} tickers")
    ...
    for ticker in tickers:
        ...
        except (GdeltRateLimited, GdeltRejected, OSError) as error:
            # Everything downloaded so far is kept.
            print(f"  {ticker}: {error}")
            print("Stopped. Run the same command again to resume.")
            break

        print(f"{ticker}: {added} new articles ({company})")
